...
tickers = ['COIN', 'TSLA', 'ABEV', 'AAPL', 'ZLAB', 'BMBL', 'RLX', 'IOT', 'QQQM', 'GNS', 'TXT', 'DE', 'CEIX', 'BBY', 'ABT', 'KALA', 'AGCO', 'ABR', 'VOO', 'GERN'] # 20 компаний для парсинга
...
print('Идет парсинг данных... Ждать 2 минуты')
st = time.time()
...
for ticker in tickers:
  ...
  finiz_url = f'https://finviz.com/quote.ashx?t={ticker}&p=d' # парсинг идёт с finviz.com
  try: # запрашиваю last_price
    summary_data = load_data(finiz_url)
    last_price = float(summary_data[5][11][10])
  except:
    print(ticker, 'last')
